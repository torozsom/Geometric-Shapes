...
static void dump_memory(void const* mem, size_t size, const size_t can_len,
    ...
    for (s = 0; s < (size + 15) / 16; s++) {
        fprintf(fp, "%04x:%c ", s * 16,
                s * 16 < can_len || s * 16 >= size - can_len ? ' ' : '*');
        for (o = 0; o < 16; o++) {
            if (o == 8)
                fprintf(fp, " ");
            if (s * 16 + o < size)
...
void mem_dump(void const* mem, const size_t size, FILE* fp) {
...
#    endif
    ...
    if (p) {
        ...
        if (size)
            fprintf(fperror, " (%d byte)", (int)size);
        fprintf(fperror, "\n");
    }
    if (a)
        print_call("\tFoglalas:\t", *a);
    if (d)
        print_call("\tFelszabaditas:\t", *d);
    if (p)
        dump_memory(p, size, CANARY_LEN, fperror);
...
static BOOL register_memory(void* p, const size_t size, call_t call) {
    initialize();
    allocated_blks++;
#    ifdef MEMTRACE_TO_FILE
    fprintf(trace_file, "%p\t%d\t%s%s", PU(p), (int)size, pretty[call.f],
            call.par_txt ? call.par_txt : "?");
    if (call.f <= 3)
        fprintf(trace_file, ")");
...
static void unregister_memory(void* p, call_t call) {
    initialize();
#    ifdef MEMTRACE_TO_FILE
    fprintf(trace_file, "%p\t%d\t%s%s", PU(p), -1, pretty[call.f],
            call.par_txt ? call.par_txt : "?");
    if (call.f <= 3)
        fprintf(trace_file, ")");
    fprintf(trace_file, "\t%d\t%s\n", call.line, call.file ? call.file : "?");
...
#    ifdef MEMTRACE_TO_MEMORY
    { /*C-blokk*/
        ...
        if (n->next) {
            ...
            if (COMP(r->call.f, call.f)) {
                ...
                if (chk < 0)
                    die("Blokk elott serult a memoria:", r->p, r->size,
                        &r->call, &call);
                if (chk > 0)
                    die("Blokk utan serult a memoria", r->p, r->size, &r->call,
                        &call);
                /*rendben van minden*/
                if (call.par_txt)
                    free(call.par_txt);
            ...
            die("Nem letezo, vagy mar felszabaditott adat felszabaditasa:", p,
                0, NULL, &call);
        }
    } /*C-blokk*/
#    endif
...
#    ifdef MEMTRACE_C
START_NAMESPACE
void* traced_malloc(const size_t size, const char* par_txt, const int line,
                    const char* file) {
    void* p;
    initialize();
...
void _set_new_handler(const std::new_handler h) {
    initialize();
    _new_handler = h;
}
...
void set_delete_call(const int line, const char* file) {
    initialize();
    delete_call =
        pack(0, "", line, file); /*func értéke lényegtelen, majd felülírjuk*/
    delete_called = TRUE;
}
...
void traced_delete(void* pu, const int func) {
    initialize();
    if (pu) {
...
void* operator new(const size_t size) THROW_BADALLOC {
...
/* Visual C++ 2012 miatt kell, mert háklis, hogy nincs megfelelő delete, bár
 * senki sem használja */
